...
var Audioplayer = videojs("recordAudio", options, function () {
...
Audioplayer.on("finishRecord", function () {
  const audioFile = Audioplayer.recordedData;
...
$("#submit").on("click", function (event) {
  ...
  const recordedFile = new File([Audioplayer.recordedData], `audiorecord.webm`);
  //   initializes an empty FormData
  let data = new FormData();
  //   appends the recorded file and language value
  data.append("recorded_audio", recordedFile);
  data.append("record_type","audio");
  //   post url endpoint
  const url = "/record/";
  console.log(Audioplayer.recordedData)
  if(Audioplayer.recordedData){
